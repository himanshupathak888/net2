    ...
    public void waitForElementToBeClickable(WebElement element) {
        try {
           ...
           // Thread.sleep(4000); // Optional: Consider replacing with explicit waiting for better practice
            ...
            System.out.println("Error: Element not clickable within " + timeoutInSeconds + " seconds. " + e.getMessage());
        }
    }
 
    
    
 // Custom wait method for element to be interactable
    public void waitForElementToBeInteractable(WebElement element) {
        try {
        	Thread.sleep(4000);
            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeoutInSeconds));
            wait.until(driver -> element.isDisplayed() && element.isEnabled());
            System.out.println("Element is interactable: " + element);
            //Thread.sleep(4000); // Optional: Replace with explicit wait for better practice
        } catch (Exception e) {
            System.out.println("Error: Element not interactable within " + timeoutInSeconds + " seconds. " + e.getMessage());
        }
    }
  ...
  //Custom wait methods2
    ...
    public void waitForVisibilityOfAllElements(List<WebElement> elements) {
        try {
            ...
            System.out.println("Error: Not all elements became visible within " + timeoutInSeconds + " seconds. " + e.getMessage());
        }
    }
